...
export default function LeadServices({ content }: { content: SiteContent }) {
  return (
    <section id="diensten" className="relative bg-slate-50 py-28 overflow-hidden">
      {/* Gradient mesh background */}
      <div className="absolute inset-0 pointer-events-none opacity-30">
        <div className="absolute top-1/4 left-1/4 w-96 h-96 rounded-full bg-indigo-200 gradient-mesh-blob" />
        <div className="absolute bottom-1/4 right-1/4 w-80 h-80 rounded-full bg-purple-200 gradient-mesh-blob" />
      </div>
      ...
      <div className="relative mx-auto max-w-6xl px-6">
        ...
        >
          <span className="text-sm font-semibold uppercase tracking-widest text-slate-400">Diensten</span>
          ...
          {content.services.map((service, i) => {
            ...
            return (
              <motion.div
                ...
                className="group relative overflow-hidden rounded-3xl border border-white/80 bg-white/70 p-10 backdrop-blur-sm transition-all duration-500 hover:-translate-y-2 hover:shadow-2xl hover:shadow-slate-200/50 hover:bg-white"
              >
                {/* Hover glow */}
                <div className="absolute inset-0 rounded-3xl bg-gradient-to-br from-indigo-50 to-purple-50 opacity-0 transition-opacity duration-500 group-hover:opacity-100" />

                <div className="relative">
                  <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-2xl bg-slate-900 text-white transition-all duration-500 group-hover:scale-110 group-hover:bg-indigo-600 group-hover:shadow-lg group-hover:shadow-indigo-200">
                    <svg className="h-7 w-7" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
                      {icon}
                    </svg>
                  </div>
                  <h3 className="text-xl font-bold text-slate-900">{service.title}</h3>
                  <p className="mt-3 text-slate-500 leading-relaxed">{service.text}</p>
                </div>
              </motion.div>
            );
          })}
