...
function FloatingIcon({ children, delay, duration, className = '' }: { children: React.ReactNode; delay: number; duration: number; className?: string }) {
  return (
    <motion.div
      ...
      className={`absolute w-10 h-10 rounded-xl bg-white shadow-lg flex items-center justify-center text-[#2563eb] ${className}`}
    >
      {children}
    </motion.div>
  );
      ...
      <div className="max-w-4xl mx-auto relative">
        <div className="rounded-3xl bg-gradient-to-br from-slate-50 to-blue-50/50 p-8 md:p-12 relative overflow-hidden">
          ...
          <div className="relative z-10 max-w-xl">
            <span className="font-subheading text-xs font-semibold text-[#2563eb] uppercase tracking-wider mb-4 block">Over ons</span>
